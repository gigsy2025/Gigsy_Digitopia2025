import { auth } from "@clerk/nextjs/server";
import type { ReactNode } from "react";
import { redirect } from "next/navigation";
import Sidebar from "./components/Sidebar";
import { getConvexServer } from "@/lib/convex/client";
import { resolveCurrentUser } from "@/lib/auth/userResolver.server";
import type {
  ConversationParticipantProfile,
  ParticipantProfileLookup,
} from "./types";

interface ConversationsLayoutProps {
  readonly children: ReactNode;
}

export default async function ConversationsLayout({
  children,
}: ConversationsLayoutProps) {
  const { userId, getToken } = await auth();

  if (!userId) {
    redirect("/sign-in");
  }

  const token = (await getToken?.({ template: "convex" })) ?? undefined;
  if (!token) {
    redirect("/sign-in");
  }

  const viewerProfile = await resolveCurrentUser();
  if (!viewerProfile) {
    redirect("/sign-in");
  }

  const convex = getConvexServer({ token });
  const sidebarData = await convex.listConversationsForUser({ limit: 25 });
  const viewerUserId = await convex.resolveViewerUserId(userId);

  const participantIds = new Set<typeof viewerUserId>();
  for (const conversation of sidebarData.conversations) {
    for (const participantId of conversation.participants) {
      if (participantId !== viewerUserId) {
        participantIds.add(participantId);
      }
    }
  }

  const participantProfiles = await convex.getUserProfiles(
    Array.from(participantIds),
  );

  const initialProfiles = participantProfiles.reduce<ParticipantProfileLookup>(
    (accumulator, profile) => {
      accumulator[profile.id] = profile;
      return accumulator;
    },
    {} as ParticipantProfileLookup,
  );

  const viewerParticipant: ConversationParticipantProfile = {
    id: viewerUserId,
    name:
      viewerProfile.name ??
      viewerProfile.username ??
      viewerProfile.email ??
      "You",
    avatarUrl: viewerProfile.avatar ?? null,
  };

  return (
    <div className="bg-background flex h-[calc(100vh-4rem)] min-h-0 w-full overflow-hidden">
      <Sidebar
        initialConversations={sidebarData.conversations}
        initialCursor={sidebarData.continueCursor}
        initialIsDone={sidebarData.isDone}
        viewerId={viewerUserId}
        viewerProfile={viewerParticipant}
        initialProfiles={initialProfiles}
      />
      <div className="border-border bg-background flex min-h-0 flex-1 flex-col border-l">
        {children}
      </div>
    </div>
  );
}
