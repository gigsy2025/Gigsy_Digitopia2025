...
export const CourseStatusSchema = z.enum(
  ["draft", "published", "archived", "coming_soon", "private"],
  {
    errorMap: () => ({
      message:
        "Course status must be draft, published, archived, coming_soon, or private",
    }),
  },
);
...
export const LessonContentTypeSchema = z.enum(
  ["text", "video", "audio", "interactive", "quiz", "assignment", "live"],
  {
    errorMap: () => ({
...
export const CourseFormatSchema = z.enum(
  ["self-paced", "instructor-led", "hybrid", "live"],
  {
    errorMap: () => ({
      message:
        "Course format must be self-paced, instructor-led, hybrid, or live",
    }),
  },
);
...
export const CoursePricingSchema = z
  .object({
    ...
    originalPrice: z
      ...
      .optional(),
    paymentType: z
      .enum(["one-time", "subscription", "per-module"])
      .default("one-time"),
  })
...
export const CourseStatsSchema = z.object({
  ...
  recentEnrollments: z
    .number()
    .int()
    .min(0, "Recent enrollments cannot be negative"),
});
...
export const LessonResourceSchema = z.object({
  ...
  fileSize: z.string().max(20, "File size description too long").optional(),
});
...
export const CourseLessonSchema = z.object({
  ...
  resources: z
    ...
    .max(10, "Maximum 10 resources per lesson"),
});
...
export const CourseModuleSchema = z.object({
  ...
  completionRequirement: z
    .enum(["all-lessons", "percentage", "assessment"])
    .default("all-lessons"),
});
...
export const CourseFAQSchema = z.object({
  ...
  category: z.enum(["general", "technical", "payment", "completion"], {
    errorMap: () => ({
      message:
        "FAQ category must be general, technical, payment, or completion",
    }),
  }),
});
...
export const CourseCertificateSchema = z.object({
  ...
  type: z.enum(["completion", "achievement", "professional"], {
    errorMap: () => ({
      message:
        "Certificate type must be completion, achievement, or professional",
    }),
  }),
  description: z
    .string()
...
export const CourseSearchSchema = z.object({
  ...
  category: CourseCategorySchema.or(z.literal("all")).default("all"),

  difficulty: CourseDifficultySchema.or(z.literal("all")).default("all"),

  priceFilter: z.enum(["free", "paid", "all"]).default("all"),
...
export const CourseEnrollmentSchema = z.object({
  ...
  enrollmentSource: z
    .enum(["web", "mobile", "api", "admin", "bulk"])
    .default("web"),
...
export type CourseCatalogResponseType = z.infer<
  typeof CourseCatalogResponseSchema
>;
