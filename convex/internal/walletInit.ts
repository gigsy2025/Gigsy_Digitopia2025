...
const createWallet = internalMutation({
  ...
  handler: async (ctx, args) => {
    ...
    if (exists) {
      ...
      throw new ConvexError(`Wallet already exists for user ${userId} with currency ${currency}`);
    }
    ...
    if (initialBalance > 0) {
      await ctx.db.insert("transactions", {
        ...
        idempotencyKey: idempotencyKey ? `${idempotencyKey}-initial` : undefined,
        ...
        try {
          ...
          if (initialBalance && initialBalance > 0) {
            await ctx.db.insert("transactions", {
              ...
              status: "COMPLETED",
              description: "Initial wallet funding",
              idempotencyKey: idempotencyKey ? `${idempotencyKey}-${currency}` : undefined,
              relatedEntityType: "system",
              relatedEntityId: `user-${userId}`,
              createdAt: Date.now(),
              createdBy: `system:wallet-init:${userId}`,
